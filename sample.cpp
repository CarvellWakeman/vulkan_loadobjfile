...
InitGraphics( )
{
	...
	objModel = LoadObjFile("resources/cube.obj");

	// Build vector struct on the heap
	VertexDataSize = objModel.vertices.size();
	struct vertex* VertexData = (struct vertex*)malloc(sizeof(struct vertex) * VertexDataSize);

	for (int i = 0; i < VertexDataSize; i++) { VertexData[i] = objModel.vertices[i]; }

	// DRAW BY INDEX
	// Build index struct on the heap
	//int IndexDataSize = objModel.indices.size();
	//int* IndexData = (int*)malloc(sizeof(int) * IndexDataSize);
	//
	//for (int i = 0; i < IndexDataSize; i++) {
	//	IndexData[i] = objModel.indices[i];
	//}
	...
	Init05MyVertexDataBuffer(  sizeof(VertexData[0])*VertexDataSize, &MyVertexDataBuffer );
	Fill05DataBuffer( MyVertexDataBuffer,	(void *) VertexData);

	// DRAW BY INDEX
	//Init05MyIndexDataBuffer(sizeof(IndexData[0])*IndexDataSize, &MyIndexDataBuffer);
	...
	Init12SpirvShader( "light-vert.spv", &ShaderModuleVertex );
	Init12SpirvShader( "light-frag.spv", &ShaderModuleFragment );
...
RenderScene( )
{
	...
	const uint32_t vertexCount = VertexDataSize;

	const uint32_t indexCount = objModel.indices.size();
	const uint32_t firstIndex = 0;
	const uint32_t vertexOffset = 0;


	// Buffers
	VkBuffer vBuffer = { MyVertexDataBuffer.buffer };
	VkBuffer iBuffer = { MyIndexDataBuffer.buffer };
...
Reset( )
{
	...
	Light.uLightColor = glm::vec4(1., 0.7, 0., 1.);
	Light.uLightSpecularColor = glm::vec4(1., 1., 1., 1.);
	Light.uShininess = 50.f;
...
UpdateScene( )
{
	...
	Misc.uMode = Mode;
	//Misc.uUseAmbient = UseAmbient;
	//Misc.uUseDiffuse = UseDiffuse;
	//Misc.uUseSpecular = UseSpecular;
	Fill05DataBuffer( MyMiscUniformBuffer, (void *) &Misc );
}
...
GLFWKeyboard( GLFWwindow * window, int key, int scancode, int action, int mods )
{
	if( action == GLFW_PRESS )
	{
		switch( key )
		{
			...
			// Ambient
			case '1':
				Misc.uUseAmbient = (Misc.uUseAmbient ==0 ? 1 : 0);
				break;
			case '5':
				if (Light.uKa < 1.0) {
					Light.uKa += 0.1;
					if (Light.uKd - 0.05 > 0.0) { Light.uKd -= 0.05; }
					if (Light.uKs - 0.05 > 0.0) { Light.uKs -= 0.05;}
				}
				break;
			case '4':
				if (Light.uKa > 0.0) {
					Light.uKa -= 0.1;
					if (Light.uKd + 0.05 < 1.0) { Light.uKd += 0.05; }
					if (Light.uKs + 0.05 < 1.0) { Light.uKs += 0.05; }
				}
				break;
			// Diffuse
			case '2':
				Misc.uUseDiffuse = (Misc.uUseDiffuse == 0 ? 1 : 0);
				break;
			case '7':
				if (Light.uKd < 1.0) {
					Light.uKd += 0.1;
					if (Light.uKa - 0.05 > 0.0) { Light.uKa -= 0.05; }
					if (Light.uKs - 0.05 > 0.0) { Light.uKs -= 0.05; }
				}
				break;
			case '6':
				if (Light.uKd > 0.0) {
					Light.uKd -= 0.1;
					if (Light.uKa + 0.05 < 1.0) { Light.uKa += 0.05; }
					if (Light.uKs + 0.05 < 1.0) { Light.uKs += 0.05; }
				}
				break;
			// Specular
			case '3':
				Misc.uUseSpecular = (Misc.uUseSpecular == 0 ? 1 : 0);
				break;
			case '9':
				if (Light.uKs < 1.0) {
					Light.uKs += 0.1;
